...
%         text(0,0,str)
    ...
    function update_model() 
        ...
        p.M=1/sqrt(1+p.w^2*p.tau^2); %amplitude ratio 
        p.phi=atan(p.w*p.tau);  
%         ah=axes('xlim',[-1,2*1/p.w],'ylim',[-15,15]);

        str=sprintf('A : %6.2f\n\nw : %6.2f\n\nm : %6.2f\n\nc : %6.2f\n\n',p.A,p.w,p.m,p.c);
        txt = uicontrol('Style','text',...
            'FontSize',fsize,...
            'Position',[175 sld_y-40 100 200],...
            'String',str);
        str=sprintf('M : %6.2f\n\nphi : %6.2f\n\n',p.M,p.phi);
        txt = uicontrol('Style','text',...
            'FontSize',fsize,...
            'Position',[100 sld_y-150 100 100],...
            'String',str);
%         show_bode()
    end
